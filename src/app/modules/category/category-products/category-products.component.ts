...
export class CategoryProductsComponent implements OnInit {
  ...
  getCategoryDetail() {
    ...
    this.categoryService.getCategory(categoryId).subscribe(
      categoryRes => {
        this.category = categoryRes.categories[0];
        if(this.category.subCategories!== null)
        {
        if (this.category.subCategories.length === 0) {
           this.categoryProductsNum = 0;
        }
       // this.category.subCategories.forEach(subCategory => {
       ////  // this.getSubCategoryProducts(subCategory.id);
       // });
      }
      else
      {
        this.category.subCategories = new CategoryDto().subCategories;
      }
    ...
    this.productService.getCategoryProducts(catId, page, 8).subscribe(
      categoryProductsRes => {
       ...
       // this.categoryProductsNum = this.categoryProductsNum + categoryProductsRes.Data.length;
        var index = this.category.subCategories.findIndex(x => x.id === catId);
        if (~index) {
          if (page == 0) {
          ...
          this.category.subCategories[index].ProductPageNum = page;
        }
